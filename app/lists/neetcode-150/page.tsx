...
import { PATTERN_META, PATTERN_ORDER, type Pattern } from "@/lib/patterns";
import { ArrowLeft } from "lucide-react";

export const dynamic = "force-dynamic";

export default async function NC150Page() {
  const problems = await getProblemsByList("neetcode150");
    ...
    <main className="container mx-auto max-w-5xl px-4 py-6 md:py-10">
      ...
      <header className="mb-8 space-y-2 border-b pb-6">
        ...
        <h1 className="font-serif-italic text-4xl font-light tracking-tight md:text-5xl">
          NeetCode 150<span className="text-primary">.</span>
        </h1>
        <p className="text-sm text-muted-foreground">
          The full breadth. Hit Blind 75 first, then expand here.
        </p>
      ...
      <div className="space-y-8">
        {PATTERN_ORDER.map((pattern) => {
          const items = byPattern.get(pattern);
